...
sys_mmap(void)
{
  ...
  // MAP_SHARED 需要文件可写；MAP_PRIVATE 不需要
      ...
      if (p->vmas[i].flags == MAP_SHARED) {
        ...
        for (uint64 va = addr; va < addr + len; va += PGSIZE) {
          ...
          if (pte && (*pte & PTE_V)) {
            ...
            ilock(ip);
            uint64 offset = va - p->vmas[i].va + p->vmas[i].offset;
