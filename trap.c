...
usertrap(void)
{
  ...
  if(r_scause() == 8){
    ...
    // scause 13 = 加载页错误, 15 = 存储页错误
    if ((scause == 13 || scause == 15) && handle_mmap_fault(stval) == 0) {
      // mmap 页错误处理成功
    } else{
      // 不是 mmap 页错误或处理失败
...
handle_mmap_fault(uint64 va)
{
  ...
  if (offset >= file_size) {
    ...
    int n = readi(ip, 0, (uint64)mem, offset, bytes_to_read);
    iunlock(ip);
